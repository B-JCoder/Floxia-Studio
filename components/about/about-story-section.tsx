  ...
  return (
    <section ref={ref} className="relative py-16 sm:py-20 lg:py-28 bg-background overflow-hidden border-t border-foreground/10">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12">
        <div className="grid lg:grid-cols-[1fr,1.6fr] gap-10 sm:gap-14 lg:gap-28 items-start">
          ...
          <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-display leading-[1.1] tracking-tight mb-6 sm:mb-10">
              From Passion to a{" "}
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {milestones.map((m, i) => (
                ...
                >
                  <span className="block font-display text-2xl sm:text-3xl text-foreground">{m.year}</span>
                  <span className="block text-[10px] sm:text-xs font-mono text-muted-foreground mt-1 uppercase tracking-widest">{m.label}</span>
                </div>
              ))}
            </div>
          </div>
